...
export async function GET() {
  try {
    ...
    if (!approvedBookings) {
      return NextResponse.json({ message: 'No approved bookings found' }, { status: 404 });
    }

    // Map the bookings to include only necessary information
    const peopleWithParkings = approvedBookings.map(booking => ({
      email: booking.email,
    ...
    console.error(error);
    return NextResponse.json({ message: 'Error fetching approved bookings' }, { status: 500 });
  }
}